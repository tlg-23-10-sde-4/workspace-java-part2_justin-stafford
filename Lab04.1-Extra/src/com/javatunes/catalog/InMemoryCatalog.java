...
public class InMemoryCatalog implements Catalog {
    ...
    public MusicItem findById(Long id) {
        ...
        for (MusicItem currentItem : catalogData) {
            if (currentItem.getId().equals(id)) {
                item = currentItem;
                break;  // break out of for loop if you find the item
            }
        }
        return item;
    }
    ...
    public Collection<MusicItem> findByKeyword(String keyword) {
        ...
        for (MusicItem item : catalogData) {
            if (item.getTitle().toLowerCase().contains(keyword.toLowerCase()) || item.getArtist().toLowerCase().contains(keyword.toLowerCase())) {
                result.add(item);
            }
        }
        return result;
...
public Collection<MusicItem> findSelfTitled () {
     ...
     * TASK: find the cheapest item with the specified genre (MusicCategory).
     */
    public MusicItem cheapItemInGenre(MusicCategory category) {
        return null;
    }
     ...
     */
    public boolean isPriceOver10(double price) {
        return false;
    }
     ...
     *  Boolean   isItemInGenre(MusicCategory category);
    */
    public boolean isItemInGenre(MusicCategory category) {
     ...
     * collections
     */
    public Collection<String> findPopTitles() {
    ...
    public Map<MusicCategory, Collection<MusicItem>> getAllGenreMap() {
        ...
        // that returns a Collection<MusicItems> for that category
        // every ENUM has a valueOf and value (static method call)- and you can for each over the array
